...
try {
    ...
    switch ($accion) {
        ...
        case 'registrarGasto':
            ...
            $gasto = $data['gasto'];
            $stmt = $pdo->prepare("INSERT INTO DSI_salon_gastos (descripcion, monto, fecha_gasto, comprobante_url, actividad_id, admin_id) VALUES (?, ?, NOW(), ?, ?, ?)");
            $actId = isset($gasto['actividadId']) ? $gasto['actividadId'] : null;
            if ($stmt->execute([$gasto['descripcion'], $gasto['monto'], $gasto['comprobanteUrl'], $actId, $adminId])) {
        ...
        case 'eliminarGasto':
            ...
            echo json_encode(['ok' => $stmt->execute([$gastoId])]);
            break;
        ...
        case 'obtenerReporteAvanzado':
            ...
            $sqlDes = "
                SELECT 
                    a.id, a.titulo,
                    (SELECT COALESCE(SUM(p.monto), 0) FROM DSI_salon_pagos p WHERE p.actividad_id = a.id AND p.confirmado = 1 AND p.fecha_pago BETWEEN :ini AND :fin) as ingresos,
                    (SELECT COALESCE(SUM(g.monto), 0) FROM DSI_salon_gastos g WHERE g.actividad_id = a.id AND g.fecha_gasto BETWEEN :ini AND :fin) as gastos
                FROM DSI_salon_actividades a
                ORDER BY a.fecha_creacion DESC
            ";
            $stmtDes = $pdo->prepare($sqlDes);
            $stmtDes->execute([':ini' => $inicioFull, ':fin' => $finFull]);
